...
\`\`\`
${e}
\`\`\``:s||(u=e);let c,l={},y;if(u.length>0&&(c={content:u},y=await this._axios.post(this.destination,c,l).catch(x=>({status:400,reason:x})),(y.status<200||y.status>399)&&console.error(`Bad Response: ${y.reason}`)),s){let x=new A({name:"file",data:e,fileName:"message.txt"});c=x.multipart,l={headers:{"Content-Type":`multipart/form-data; boundary=${x.boundary}`}},await Dt(this._rateLimit||et),y=await this._axios.post(this.destination,c,l).catch(Q=>({status:400,reason:Q})),(y.status<200||y.status>399)&&console.error(`Bad Response: ${y.reason}`)}return{destination:this.destination,channelName:this.channelName,result:!0}}};D.version=F;return Ot;})();
